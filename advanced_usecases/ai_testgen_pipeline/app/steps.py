...
def run_agent(context: WorkflowContext, repo_path: Path, pytest_feedback: str | None = None) -> int:
    run = context.repository.get_run(context.run_id)
    if run is None:
        raise KeyError(f"run not found: {context.run_id}")
    result = context.agent_runner.generate_tests(repo_path, run.request, pytest_feedback=pytest_feedback)
    fields = {
...
def maybe_create_pr(context: WorkflowContext, repo_path: Path, pytest_exit_code: int) -> str | None:
    ...
    try:
        ...
        pr_title = None
        pr_body = None
        if run.request.source_pr_number is not None:
            branch_name = f"ai-tests/pr-{run.request.source_pr_number}"
            pr_title = f"AI Generated Tests for PR #{run.request.source_pr_number}"
            pr_body = (
                f"Generated by the AI test generation pipeline for source PR "
                f"#{run.request.source_pr_number}."
            )
        ...
        ci_status = context.github_client.get_pr_checks(repo_path, branch)
